RunSims = function(alleles, allelefreqs, popsize, simyears, survival, agecap, reps, structK, levels){
  ...
  for (n in 1:length(popsize)){
    ...
    for(a in 1:length(agecap)){
      ...
      for (j in 1:reps){
        #het matrix
        hetmatrix = matrix(nrow = simyears, ncol = 4) 
        colnames(hetmatrix) = c("rep", "year", "maxage", "H")
        
        #fst matrix 
        fstmatrix = matrix(nrow = simyears, ncol = 4)
        colnames(fstmatrix) = c("rep", "year", "maxage", "fst")
        
        #dapc matrix 
        dpcmatrix = matrix(nrow = simyears, ncol = 5)
        colnames(dpcmatrix) = c("rep", "year", "maxage", "OintoO", "CintoO")
        ...
        for(g in 1:(simyears)){
          ...
          if(g==1){
            ...
            current = matrix(nrow=popsize[n], ncol=numloci*2)
            ...
            #assign age randomly, from 1 to max age for sim
            age = sample(seq(1, agecap[a], 1), size = nrow(current), replace=TRUE)
          ...
          current[,(numloci*2)+1] = current[,(numloci*2)+1] + 1
          
          #determine 'random' mortality, mark those selected as dead (0)
          tokill = sample(seq(1, nrow(current), 1), size = round((1-survival)*nrow(current)), replace=FALSE)
          current[tokill,(numloci*2)+2] = 0
          
          #mark individuals older than agecap as dead (0)
          current[current[,(numloci*2)+1]>agecap[a],(numloci*2)+2] = 0
          
          #generate list of dead
          toreplace = which(current[,(numloci*2)+2]==0)
          current[toreplace,] = NA
          ...
          for(l in lcols){
            ...
            alleles[[locus]]     = sort(as.numeric(unique(c(previous[,l], previous[,l+1]))))
            
            #generate list of allele frequencies
            allelefreqs[[locus]] = as.numeric(sort(table(c(previous[,l], previous[,l+1]))) / (nrow(previous[!is.na(previous[,1]),,drop=FALSE])*2))
            
            #replace rows of 'dead' indivuals with newly sampled allele frequencies
            for(r in 1:length(toreplace)){
              if(length(alleles[[locus]])==1){
                current[toreplace[r], l]   = alleles[[locus]]
                current[toreplace[r], l+1] = alleles[[locus]]
              }else{
                current[toreplace[r], l]   = sample(x=alleles[[locus]], size=1, prob=allelefreqs[[locus]], replace=TRUE)
                current[toreplace[r], l+1] = sample(x=alleles[[locus]], size=1, prob=allelefreqs[[locus]], replace=TRUE)
              }
            }
            locus = locus + 1  
            
            #mark age = 0
            current[toreplace, (numloci*2)+1]  = 0
            
            #reset to alive
            current[toreplace, (numloci*2)+2]  = 1 
          }
          
          #### measure population parameters of interest ####
          
          #first, create variables to hold current year and original population data  #######DO WE WANT THIS BEFORE OR AFTER H CALCS?
          ...
          hetmatrix[g,1] = j              #replicate
          hetmatrix[g,2] = g              #year
          hetmatrix[g,3] = agecap[a]      #max age
          hetmatrix[g,4] = mean(hets[,1]) #het
          ...
          fstmatrix[g,1] = j                                    #replicate
          fstmatrix[g,2] = g                                    #year
          fstmatrix[g,3] = agecap[a]                            #max age
          fstmatrix[g,4] = fst$std_stats$Fst[length(lnames)+1]  #fst
          ...
          dpcmatrix[g,1] = j                                    #replicate
          dpcmatrix[g,2] = g                                    #year
          dpcmatrix[g,3] = agecap[a]                            #max age
          dpcmatrix[g,4] = mean(dapc.out$posterior[1:nrow(original), 2])                                   #Original into original
          dpcmatrix[g,5] = mean(dapc.out$posterior[(nrow(original)+1):(nrow(current)+nrow(original)), 2])  #Current into original
          ...
          if(!is.na(match(g, levels))){
            ...
            torecord = rbind(as.matrix(j), as.matrix(g), as.matrix(agecap[a]), as.matrix(E$mean.ln.k))
            torecord = t(torecord)
        ...
        write.table(hetmatrix,paste(outdir, "het", popsize[n], ".csv", sep = ""),sep=",",col.names=FALSE, row.names=FALSE, append=TRUE)
        write.table(fstmatrix,paste(outdir, "fst", popsize[n], ".csv", sep = ""),sep=",",col.names=FALSE, row.names=FALSE, append=TRUE)
        write.table(dpcmatrix,paste(outdir, "dpc", popsize[n], ".csv", sep = ""),sep=",",col.names=FALSE, row.names=FALSE, append=TRUE)
        write.table(evamatrix,paste(outdir, "eva", popsize[n], ".csv", sep = ""),sep=",",col.names=FALSE, row.names=FALSE, append=TRUE)
      }#j
    }#a
  }#n
}#RunSims
